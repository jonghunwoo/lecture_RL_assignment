...
For a public or student-facing Git repository, the instructor solutions should be maintained in a separate private repository or excluded using an appropriate repository-management strategy.
...
```bash
python starter_code/assignment_01_smsp_mdp_baseline/smsp_baseline.py
```

### Assignment 2

```bash
python starter_code/assignment_02_policy_learning_smsp/smsp_q_learning_starter.py
```

### Assignment 3

```bash
python starter_code/assignment_03_constraint_aware_jssp/jssp_masking_starter.py
```
...
Exact reproducibility may still depend on the operating system, package version, hardware, and GPU operations. Students should report the execution environment used for their experiments.
...
A single-file implementation is also acceptable for small assignments when the code remains readable.
...
Model checkpoints may be committed only when explicitly required.
